...
const dataHead = [
  { name: '序号', dataIndex: 'dataIndex', style: { width: '10%' } },
  { name: '街道名称', dataIndex: 'A', style: { width: '20%' } },
  {
    name: '第一针新增', dataIndex: 'C', style: { width: '24%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '第一针累计', dataIndex: 'D', style: { width: '24%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
