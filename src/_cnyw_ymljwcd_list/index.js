...
const dataHead = [
  { name: '类型', dataIndex: 'name', style: { width: '36%' } },
  {
    name: '第一针', dataIndex: 'A', style: { width: '20%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '第二针', dataIndex: 'B', style: { width: '20%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '合计', dataIndex: 'C', style: { width: '20%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  // {
  //   name: '排名', dataIndex: 'D', style: { width: '16%' }
  // }
]
