...
const bianjie = {
  "bianjie": {
      ...
      'messageType': 'clear',
      'flash': false,
      'params': { 'default': false, 'pinType': 'bianjie', 'messageType': 'clear', 'flash': false },
      'state': true
  }
}
                ...
                <div style={{ width, height, transform: `scale(${!mapType || mapType === 'arcgis' ? 1 : this.state.scale})` }}>
                    {!mapType || mapType === 'arcgis' ?
                        <ArcgisMap 
                          ...
                          datas={{ ...this.state.pointsData, ...xzzxData, ...bianjie }} /> :
                        <LbsAmap 
                          code={`${code}`} 
                          zoom={zoom} 
                          center={{ x, y }} 
                          datas={{ ...this.state.pointsData, ...xzzxData }} 
                          mapCenter={this.state.mapCenter} 
                          onVideoClick = {this.state.handlers.onClick}
                          shipindata = {this.shipindianwei} />
                    }
                </div>
            </div>
        );
