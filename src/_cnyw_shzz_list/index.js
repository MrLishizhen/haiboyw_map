...
const dataHead = [
  { name: '序号', dataIndex: 'dataIndex', style: { width: '3%' } },
  { name: '类型', dataIndex: 'btype', style: { width: '3%' } },
  { name: '单位名称', dataIndex: '单位名称', style: { width: '12%' }, tooltip: true },
  { name: '统一社会信用代码', dataIndex: '统一社会信用代码', style: { width: '10%' } },
  { name: '业务范围', dataIndex: '业务范围', style: { width: '10%' }, tooltip: true },
  { name: '业务主管单位', dataIndex: '业务主管单位', style: { width: '13%' } },
  { name: '住所地址', dataIndex: '住所地址', style: { width: '10%' }, tooltip: true },
  { name: '法定代表人', dataIndex: '法定代表人', style: { width: '5%' } },
  { name: '联系电话', dataIndex: '联系电话', style: { width: '6%' }, tooltip: true },
  { name: '登记日期', dataIndex: '登记日期', style: { width: '5%' } },
  {
    name: '证书有效期终止日期', dataIndex: '证书有效期终止日期', style: { width: '9%' }, render: (data) => {
      let result = data;

      try {
        if (moment(data, 'YYYY-MM-DD').isBefore(moment())) {
          return <span style={{ color: 'red' }}>{data}</span>;
        } else if (moment(data, 'YYYY-MM-DD').diff(moment(), 'days') < 90) {
          return <span style={{ color: '#FEC227' }}>{data}</span>;
        }
      } catch (e) {
        console.error(e);
      }
  ...
  { name: '2019年检结论', dataIndex: '2019年检结论', children: [], style: { width: '6%' } },
  { name: '规范化评估等级', dataIndex: '规范化评估等级', children: [], style: { width: '7%' } }
]
