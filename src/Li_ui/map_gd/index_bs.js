...
export default class Map_gd extends Component {
    ...
    setMap = (that) => {
        ...
        that.map = new AMap.Map(this.id, {
            resizeEnable: true,
            center: [data?.lng || '121.489431', data?.lat || '31.405242'],
            showLabel: false,
        ...
        that.map.on("complete", function () {
            ...
            AMap.plugin('AMap.DistrictSearch', () => {
                ...
                // 搜索所有省/直辖市信息
                this.districtSearch.search(data.name || '310113', (status, result) => {
        ...
        if (bounds) {
            for (let i = 0, l = bounds.length; i < l; i++) {
                ...
                let polygon = new AMap.Polygon({
                    zIndex: 110,
                    map: that.map, // 指定地图对象
                    strokeWeight: name === '宝山区' ? 4 : 1, // 轮廓线宽度
                    path: bounds[i], //轮廓线的节点坐标数组
                    fillOpacity: 1, //透明度
                    fillColor: name === '宝山区' ? "#102645" : '#102645', //填充颜色
                    strokeColor: name === '宝山区' ? '#3c70b9' : '#94C4FB', //线条颜色
                })
