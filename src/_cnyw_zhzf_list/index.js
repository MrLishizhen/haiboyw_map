...
const dataHead = [
  ...
  { name: '执法领域名称', dataIndex: '执法领域名称', style: { width: '10%' } },
  { name: '法律法规名称', dataIndex: '法律法规名称', style: { width: '14%' } },
  { name: '名称', dataIndex: '名称', style: { width: '10%' } },
  { name: '违反条款', dataIndex: '违反条款', style: { width: '14%' } },
  { name: '法律法规规章规定', dataIndex: '法律法规规章规定', style: { width: '14%' } },
  { name: '依据条例', dataIndex: '依据条例', style: { width: '14%' } },
  { name: '法律责任适用条款', dataIndex: '法律责任适用条款', style: { width: '18%' } }
]
          ...
          <tbody id='shzz-tbody' onClick={this.onClose}>
            {
              [...newData].slice(0, pageSize).map((item, i) => {
                return (
                  <TrItem key={i} columns={columns} dataHead={dataHead} i={i} item={item} parentRef={this.ref} tableHead={tableHead} tooltipFunction={this.tooltip}
                    activeIndex={this.state.activeIndex} changeParentState={(value) => this.setState(value)} />
                )
              })
            }
          </tbody>
...
class TrItem extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.activeIndex !== this.props.i) {
  ...
  render() {
    ...
    const { columns, dataHead, i, item, parentRef, tableHead } = this.props;
    const active_line = visible ? styles.active_line : styles.line;

    return (
      <Fragment>
        <tr className={active_line} onClick={() => this.setState({ visible: true })} onMouseOver={() => this.props.changeParentState({ activeIndex: i })}>
          {
            columns.map((key, j) => {
              ...
              return (
                !key.hidden ?
                  <td key={j} className={active_color} style={{ ...tableHead[j].style }}>
                    <TdItem dataHead={dataHead} item={item} dataKey={key} i={i} j={j} parentRef={parentRef} />
                  </td> : null
              );
            })
          }
        </tr>
        {visible && this.props.activeIndex === i &&
          <div className={styles.pop_card_class} style={{ left: '6%', top: 160 }} /*onMouseLeave={() => this.setState({ visible: false })}*/>
            <div className={styles.pop_box}>
              <table style={{ width: '100%' }}>
                <tbody>
                  {dataHead.map(head => {
                    if (head.dataIndex !== 'dataIndex') {
                      return (
                        <tr key={head.dataIndex}>
                          <td style={{ width: '16%' }}>{head.name}：</td>
                          <td style={{ height: 'auto', lineHeight: 2, overflow: 'unset', textAlign: 'left', width: '82%', whiteSpace: 'pre-wrap' }}>{item[head.dataIndex] ? item[head.dataIndex].replace(/\n|\s+/g, '') : '-'}</td>
                        </tr>
                      );
                    }
                  })}
    ...
    return (
      <Fragment>
        {dataKey.tooltip ?
          ...
          // </Tooltip> :
          <div onMouseOver={() => this.setState({ visible: true })} onMouseOut={() => this.setState({ visible: false })} className={styles.pop_card_class_wrapper}>
            {visible &&
              <div className={[styles.pop_card_class, styles[`drop_down_${j}`]].join(' ')} style={{ top }}>
                {tooltipFunction(dataHead, item, dataKey.title)}
              </div>
            }
            {typeof dataKey.render === 'function' ? dataKey.render(item[dataKey.title]) : item[dataKey.title]}
          </div> :
          typeof dataKey.render === 'function' ? dataKey.render(item[dataKey.title]) : dataKey.title === 'dataIndex' ? `${i + 1}` : item[dataKey.title]
        }
      </Fragment>
    );
  }
