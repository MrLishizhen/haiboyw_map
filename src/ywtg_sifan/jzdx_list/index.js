...
const dataHead = [
  ...
  { name: '姓名', dataIndex: 'B', style: { width: '10%' } },
  { name: '年龄', dataIndex: 'C', style: { width: '10%' } },
  { name: '性别', dataIndex: 'D', style: { width: '10%' } },
  { name: '街道', dataIndex: 'E', style: { width: '10%' } },
  { name: '居住地址', dataIndex: 'F', style: { width: '29%' } },
  { name: '罪名', dataIndex: 'G', style: { width: '25%' } }
]
...
class Index extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    try {
      if (!isEqual(this.getDataProvider(this.props), this.getDataProvider(nextProps))) {
  ...
  */
  ...
  trItemClick(data) {
    ...
    console.log('POST', JSON.stringify({ "type": "POP_UP", "flag": "GRIDFLAG", "pinType": `jzdx_${E}`, "id": `${ID}` }));
    window.postMessage({ "type": "POP_UP", "flag": "GRIDFLAG", "pinType": `jzdx_${E}`, "id": `${ID}` }, '*')
  }
          ...
          <tbody id='shzz-tbody' onClick={this.onClose}>
            {
              [...newData].slice(0, pageSize).map((item, i) => {
                ...
                return (
                  <tr key={i} className={active_line} onClick={this.trItemClick.bind(this, item)}>
                    {
                      columns.map((key, j) => {
