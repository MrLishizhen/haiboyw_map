...
const dataHead = [
  ...
  { name: '疫苗品牌', dataIndex: 'A', style: { width: '13%' } },
  {
    name: '上一日库存数', dataIndex: 'B', style: { width: '13%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '当日接种数', dataIndex: 'C', style: { width: '18%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '当日完成调拨总数', dataIndex: 'D', style: { width: '18%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '当日报废数', dataIndex: 'E', style: { width: '16%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '当日库存数', dataIndex: 'F', style: { width: '16%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt)
    }
  }
