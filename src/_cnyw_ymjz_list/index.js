...
const dataHead = [
  ...
  {
    name: '接种点', dataIndex: 'A', style: { width: '14%' }, tooltip: true, render: (text, record) => {
      const { B } = record || {};
  ...
  {
    name: '10:30', dataIndex: 'C', style: { width: '8%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '12:30', dataIndex: 'D', style: { width: '8%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '14:30', dataIndex: 'E', style: { width: '8%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '16:30', dataIndex: 'F', style: { width: '8%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '19:00', dataIndex: 'G', style: { width: '8%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '22:00', dataIndex: 'L', style: { width: '8%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
      ...
      <div ref={this.ref} style={{ width: '100%', height: '100%', position: 'relative' }} className={styles.table_layout_case}>
        <table style={{ width: '100%' }}>
          ...
          <tbody id='shzz-tbody' onClick={this.onClose}>
            {
              [...[...newData].slice(0, pageSize), result].map((item, i) => {
    ...
    return (
      <Fragment>
        {dataKey.tooltip ?
          ...
          typeof dataKey.render === 'function' && !item.flag ? dataKey.render(item[dataKey.title], item) : dataKey.title === 'dataIndex' ? item[dataKey.title] || `${i + 1}` : item[dataKey.title]
        }
      </Fragment>
    );
  }
