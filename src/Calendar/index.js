...
class Index extends React.Component {
  ...
  changeWindow = () => {
    ...
    const cal_width = parentNode && (parentNode.clientWidth * 0.078);
    const cal_height = parentNode && (parentNode.clientHeight * 0.2);
    this.setState({
      cal_width,
      cal_height
    })
  ...
  render() {
    ...
    const popupStyle = { maxWidth: 300, minWidth: 240, top: 48, width: cal_width, height: cal_height };
    return (
      <div style={{ width: '100%', height: '100%', position: 'relative' }} ref={node => this.node = node}>
        <div style={{ paddingTop: 0, height: 0 }}>
          <DatePicker
            locale={locale}
            size='large'
            ...
            showToday={false}
            style={{ background: 'transparent', width: '1px', border: 'transparent', outline: 'none' }}
        ...
        <div
          ...
          <img style={{ cursor: 'pointer', width: 16 }} src={icon} />
        </div>
      </div>
    )
  }
