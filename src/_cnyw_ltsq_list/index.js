...
const dataHead = [
  ...
  { name: '所属区域', dataIndex: 'streetName', style: { width: '19%' } },
  { name: '地址', dataIndex: 'address', style: { width: '19%' } },
  { name: '商业综合体及重要景点', dataIndex: 'importantPoint', style: { width: '20%' }, tooltip: true },
  {
    name: '当前人数', dataIndex: 'passengerFlow', style: { width: '20%' }, render: (text, record = {}) => {
      let { saturationQuantity, passengerFlow } = record;
      saturationQuantity = parseFloat(saturationQuantity);
      passengerFlow = parseFloat(passengerFlow);
      let color = 'rgb(0, 250, 170)';

      if (!isNaN(saturationQuantity) && !isNaN(passengerFlow)) {
        if (saturationQuantity < passengerFlow) {
          color = '#dd4b78'
        }
      }
  ...
  { name: '饱和客流量', dataIndex: 'saturationQuantity', hidden: true },
