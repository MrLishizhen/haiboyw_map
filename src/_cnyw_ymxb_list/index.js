...
const dataHead = [
  ...
  { name: '3日内市级下拨疫苗品牌', dataIndex: 'A', style: { width: '39%' } },
  {
    name: '当日', dataIndex: 'B', style: { width: '15%' },
    colRender: (date) => {
      if (date) {
        return moment(date, 'YYYYMMDD').format('MM月DD日');
      }
      return '当日'
    },
    render: (text, record) => {
  ...
  {
    name: '次日', dataIndex: 'C', style: { width: '15%' },
    colRender: (date) => {
      if (date) {
        return moment(date, 'YYYYMMDD').add(1, 'days').format('MM月DD日');
      }
      return '次日'
    },
    render: (text, record) => {
  ...
  {
    name: '第3日', dataIndex: 'D', style: { width: '15%' }, 
    colRender: (date) => {
      if (date) {
        return moment(date, 'YYYYMMDD').add(2, 'days').format('MM月DD日');
      }
      return '第3日'
    },
    render: (text, record) => {
