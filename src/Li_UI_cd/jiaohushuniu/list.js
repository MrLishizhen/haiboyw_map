...
export default class List extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!isEqual(this.props, nextProps)) {
      ...
      if (!isEqual(this.props.resetFlag, nextProps.resetFlag)) {
        this.setState({searchValue: nextProps.searchValue, current: 1, filter: {}}, () => this.getDatalist())
      }
    }
  }
  ...
  */
  ...
  getDatalist() {
    ...
    const { type, city,gteDate,lteDate,action,isSlaOverTime, flowStatus, $class_1$, flowSource, ...extra } = filter;
  ...
  render() {
    ...
    dataList=[
      {
        $class_1$:'卫生计生',
        questionTitle:'吴先生[咨询]其他',
        hadress:'青羊区',
        creatDate:'12h',
        workingOnTime:'12h',//在办时长
        flowStatus:"100",
        isSlaOverTime:true
      },{
        $class_1$:'卫生计生',
    ...
    const tableWidth = ['8%','8%','8%','13%','30%','22%','14%','10%']
    return (
        <div className={style.table_wrapper}>
          <div className={style.table_columns}>
            ...
            <div className={style.table_columns_cell} style = {{width: tableWidth[5]}}>事件名称</div>
            <div className={style.table_columns_cell} style = {{width: tableWidth[5]}}>事件所在地</div>
            <div className={style.table_columns_cell} style = {{width: tableWidth[3]}}>在办时长</div>
            <div className={style.table_columns_cell} style = {{width: tableWidth[3]}}>受理时间</div>
            <div className={style.table_columns_cell}style = {{width: tableWidth[6]}}>
              事件状态
              <Select
                  size='large'
                  allowClear={true}
                    ...
                    return (
                        <React.Fragment key={item.id}>
                          <div className={style.table_row} onClick={() => { if (typeof this.props.onDialogEvent === 'function') this.props.onDialogEvent(item) }}>
                            ...
                            <div className={style.rows_cell} style = {{width: tableWidth[5]}} title = {item['questionTitle']}>{item['questionTitle']}</div>
                            {/* </Tooltip> */}
                            ...
                            <div className={style.rows_cell} style = {{width: tableWidth[3]}}>{item.creatDate}</div>
                            <div className={style.rows_cell} style = {{width: tableWidth[6]}}>{codes[item.flowStatus] || ''}</div>
