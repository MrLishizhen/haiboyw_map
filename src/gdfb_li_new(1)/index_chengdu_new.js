...
export default class index extends Component {
    constructor(props) {
        ...
        // dataQuery=[{name:'东津新区2',count: 12345,ycl:123,wcl:123,zb:58,},{name:'东津新区1',count: 12345,ycl:123,wcl:123,zb:58,},{name:'东津新区',count: 12345,ycl:123,wcl:123,zb:58,}]
        ...
        let fIndex = 4;
        let endIndex = newData.length-1;
        if(dataQuery.length>(fIndex*3)){
            newData.push(...newData.slice(0, fIndex));
        }
    ...
    generateSwiper = () => {
        let {data,fIndex=0} = this.state;
        if(data.length>(fIndex*3)){
            this.setIntervalfunc= setInterval(()=>{
                ...
                this.setState({
                    ...
                    if(index===newData.length-fIndex){
                        setTimeout(()=>{
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        if (!isEqual(dataProvider, this.props.dataProvider)) {
            if (dataProvider && dataProvider.length === 1 && dataProvider[0].series) {
                ...
                this.setState({
                    data: dataQuery, newData: newData,sdIndex:0,endIndex:endIndex
                }, () => {
    ...
    render() {
        ...
        let { data = [], newData = [],sdIndex=0,fIndex=0,endIndex=0} = this.state;
        if (!Array.isArray(data) || data.length === 0 || data[0] === '') {
            return "";
        }

        return (
            <div className={styles.box} style={{height:fIndex*160}} onMouseEnter={this.mouseoveFunc} onMouseLeave={this.mouseoutFunc}>
                    ...
                    >
                        {
                            newData.map((child, index) => {
                                    ...
                                    return (<div className={'box-item'}
                                            ...
                                            <div className={'names'}>
                                                ...
                                                <span className={'countTitle'}>总计: <i
                                                    className={'count'}>{this.format(item?.total || 0)}</i></span>
                                            </div>
                                            <div className={'box-content'}>
                                                <div className={'yc-box'}>
                                                    <p className={'yc-p'}>
                                                        <i>已处理</i>
                                                        <span>{this.format(item?.todayTotalDealed || 0)}</span>
                                                    </p>
                                                    <p className={'yc-p wc'}>
                                                        <i className={'wcl-i'}>处理中</i>
                                                        <span>{this.format(item?.todayTotalDealing || 0)}</span>
                                                    </p>
                                                </div>
                                                <div className={'echarts-box'}>
                                                    <Item dataProvider={[item?.todayTotalDealingRate || 0]}></Item>
                                                </div>
                                            </div>
                                        </div>
                                    )
                                ...
                                return (<div key={index.toString()} style={{
                                    ...
                                    alignItems: 'flex-start',
                                    background: `url(${index===endIndex ? zx : ''}) no-repeat center 155px/ 90%`
                                }}>
                                    {children}
                                </div>)
                            })
