...
export default class Index extends React.Component {
    ...
    render() {
        ...
        const newData = [...data, BASIC_DATA, TOTAL_DATA, MAX_DATA];
                        ...
                        {[...dataHead].slice(0, 1).map((head, i) => {
                            return (
                                <tr key={i} style={{ borderBottomColor: '#023a6a' }}>
                                    <td style={{ backgroundColor: 'rgba(5, 91, 169, 0.5)', color: '#00f0ff', fontWeight: 'bolder', width: 360 }}>
                                        {head.name}
                                    </td>
                                    {newData && newData.length > 0 ?
                                        newData.map((item, j) => {
                                            return (
                                                <td key={j} style={!head.dataIndex || head.dataIndex === 'NAME' ? { backgroundColor: 'rgba(5, 91, 169, 0.5)', color: '#00f0ff', fontWeight: 'bolder', width: 360 } : { width: 360 }}>
                                                    {head.dataIndex ?
                                                        item.flag ?
