...
const TableItem = props => {
  ...
  return (
    <div style = {{width: 780, height: 138, position: 'absolute', cursor: 'pointer', marginBottom: 0, ...style}} onClick = {onClick}>
          ...
          this.state.rows.map((el, i) => {
            ...
            return (
              <TableItem 
                key = {i} 
                style = {style} 
                onClick = {() => this.state.handlers.onClick && this.state.handlers.onClick({...data})}
                {...data}/>
            )
          })
        }
