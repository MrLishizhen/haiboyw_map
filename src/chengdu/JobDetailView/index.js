...
const FormItem = props => {
  ...
  const hStyle = value === ''? {height: 80}: {};
  return (
    <div style = {{display: 'flex', alignItems: 'center', fontSize: 48}}>
      <div style = {{color: '#87D4FF', marginRight: 32}}>{title}</div>
      <div title = {value} style = {{flex: 1,  borderRadius: 8, background: '#5EA0FF30', color: 'white', lineHeight: '80px', padding: '0 10px', ...style, ...hStyle}}>{value}</div>
    </div>
  )
}
    ...
    return (
      <div style = {{background: `url(${bg})`, width: 1538, height: 2160, position: 'relative'}}>
        <div style = {{position: 'relative'}}>
          <div style = {{ background: `url(${headerbg}) no-repeat`, width: 916, height: 120, top: 52, left: 80, position: 'relative'}}>
          ...
          <div style={{overflowX: 'hidden', overflowY: 'scroll', height: '100%', position: 'relative'}}>
            ...
            <div style = {{ marginTop: 50, marginLeft: 80, width : '100%'}}>
              <div style = {{display: 'flex', alignItems: 'flex-end', flexDirection: 'row', position: 'relative'}}>
                ...
                <div style={{display: 'flex', width: 500, fontSize: 36, color: 'white', position: 'relative', marginLeft: 20, marginBottom: 5}}>
                    <div style={{display: 'flex', width: '100%', fontSize: 36, color: 'white', marginLeft: 20}}>
