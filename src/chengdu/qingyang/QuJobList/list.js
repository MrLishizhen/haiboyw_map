...
export default class List extends PureComponent {
  ...
  componentDidMount() {
    ...
    // this.setState({filter: {...this.state.filter, nodeId: this.props.filterParam}}, () => this.getDatalist())
    
    // this.timer = setInterval(() => {
    //   this.getDatalist();
    // }, 1000 * 60 * 1);

    const extraParams = this.props.style? this.props.style.filter: {};
    this.setState({filter: {...this.state.filter}, extraParams: {...extraParams}}, () => this.getDatalist());
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (!isEqual(this.props, nextProps)) {
      ...
      if (!isEqual(this.props.token, nextProps.token)) {
        ...
        this.setState({filter: {...this.state.filter, nodeId: nextProps.filterParam}}, () => this.getDatalist());
      }

      if (!isEqual(this.props.filterParam, nextProps.filterParam)) {
        this.setState({filter: {...this.state.filter, nodeId: nextProps.filterParam}}, () => this.getDatalist());
      }

      if (!isEqual(this.props.style, nextProps.style)) {
        const extraParams = nextProps.style? nextProps.style.filter: {};
        console.log("event list nextProps:", nextProps)
        this.setState({filter: {...this.state.filter}, extraParams: {...extraParams}}, () => this.getDatalist());
      }
    }
  }
  ...
  getDatalist = props => {
    const {addrIp} = this.props;
    if (!addrIp || !this.token) return;
    const { current, pageSize, searchValue, filter = {}, extraParams = {} } = this.state;
    const { type, gteDate,lteDate,action,isSlaOverTime, flowStatus, $class_1$, $urgency$, flowSource, nodeId, days = 0, ...extra } = filter;
