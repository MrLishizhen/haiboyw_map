...
const eventTypeUrl =
    ...
    'http://10.1.17.17:9527/cmdbApp/api/cidata/getMultiLevelData';

export default class List extends PureComponent {
    constructor(props) {
        super(props);
        this.region = this.getQueryVariable('region') || '成都';
    ...
    */
    getEventStatus() {
        ...
        axios.post(
            ...
            'http://10.1.17.17:9527/userOrg/api/org/dict/dictByCodeGroup', params, {
                headers: {
                    checkToken: 'no'
                }
            }).then(({ data }) => {
    ...
    */
    ...
    getDatalist() {
        ...
        axios.post(
            ...
            'http://10.1.17.17:9527/itsmApp/visData/getFlowInfoList', params, {
                headers: {
                    checkToken: 'no'
                }
            }).then(({ data: res }) => {
                                ...
                                return (
                                    <React.Fragment key={item.id}>
                                        <div className={style.table_row} onClick={() => { if (typeof this.props.onDialogEvent === 'function') this.props.onDialogEvent(item.flowNo) }}>
                                            ...
                                            <div className={style.rows_cell}>{this.region === '成都市' ? item.city : item.street}</div>
                                            {/* </Tooltip> */}
