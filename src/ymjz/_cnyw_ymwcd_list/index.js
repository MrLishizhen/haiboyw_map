...
const dataHead = [
  { name: '截止日期', dataIndex: 'A', style: { width: '26%' } },
  {
    name: '阶段性指标', dataIndex: 'B', style: { width: '24%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '累计完成数', dataIndex: 'C', style: { width: '24%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '完成进度', dataIndex: 'D', style: { width: '24%' }
  }
]
