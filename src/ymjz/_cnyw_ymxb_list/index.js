...
const dataHead = [
  { name: '序号', dataIndex: 'ID', style: { width: '4%' } },
  { name: '接种门诊简称', dataIndex: 'A', style: { width: '10%' } },
  {
    name: '北生', dataIndex: 'B', style: { width: '7%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '科兴', dataIndex: 'C', style: { width: '7%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '武生', dataIndex: 'D', style: { width: '7%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '科兴-2人份', dataIndex: 'E', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '北生-2人份', dataIndex: 'F', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt)
    }
  },
  {
    name: '康希诺', dataIndex: 'G', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt)
    }
  },
  {
    name: '科兴（3岁以上）', dataIndex: 'H', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt)
    }
  },
  {
    name: '北生-2人份（3岁以上）', dataIndex: 'I', style: { width: '11%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt)
    }
  },
  {
    name: '北生（3岁以上）', dataIndex: 'J', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt)
    }
  },
  {
    name: '总计', dataIndex: 'K', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt)
    }
  }
...
class Index extends React.Component {
  ...
  */
  ...
  render() {
    ...
    let date = data && data[0] ? data[0].E : undefined;
    ...
    return (
      <div style={{ width: '100%', height: '100%', overflowX: 'auto' }}>
        <div ref={this.ref} style={{ width: 6200, height: '100%', position: 'relative' }} className={styles.table_layout_case}>
                      ...
                      !item.hidden ?
                        <td key={index} style={{ color: color, ...item.style }}>
                          <div className={styles.line_flex} onClick={this.state.order.field === item.name ? this.onOrder.bind(this, item.name) : this.onDropDown.bind(this, item.name)}>
                            <div>{item.colRender ? item.colRender(date) : item.name}</div>
                            {
