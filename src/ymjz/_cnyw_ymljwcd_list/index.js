...
const dataHead = [
  { name: '街道镇', dataIndex: 'A', style: { width: '10%' } },
  {
    name: '第一针', dataIndex: 'B', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '康希诺', dataIndex: 'C', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '第二针', dataIndex: 'D', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '全程接种', dataIndex: 'E', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '接种总数', dataIndex: 'F', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '第一针', dataIndex: 'G', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '康希诺', dataIndex: 'H', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '第二针', dataIndex: 'I', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '全程接种', dataIndex: 'J', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '接种总数', dataIndex: 'K', style: { width: '9%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  }
    ...
    return (
      ...
      <div ref={this.ref} style={{ width: '100%', height: '100%', position: 'relative' }} className={styles.table_layout_case}>
        <table style={{ width: '100%' }}>
          <thead>
            <tr>
              <th style={{textAlign: 'center'}} width='10%' rowSpan={2}>街道镇</th>
              <th style={{textAlign: 'center'}} width='45%' colSpan={5}>18周岁及以上人群当日接种数</th>
              <th style={{textAlign: 'center'}} width='45%' colSpan={5}>12-17周岁人群当日接种数</th>
            </tr>
            <tr>
              {
                Array.isArray(tableHead) && tableHead.map((item, index) => {
                  ...
                  return (
                    !item.hidden ?
                      <th key={index} style={{ color: color, ...item.style }}>
