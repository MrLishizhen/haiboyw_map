...
const dataHead = [
  { name: '序号', dataIndex: 'ID', style: { width: '10%' } },
  { name: '街道名称', dataIndex: 'A', style: { width: '30%' } },
  {
    name: '当日新增第二针', dataIndex: 'C', style: { width: '29%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '累计新增第二针', dataIndex: 'D', style: { width: '29%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  // {
  //   name: '任务完成率', dataIndex: 'E', style: { width: '20%' }
  // }
]
...
class Index extends React.Component {
  ...
  */
  ...
  render() {
    ...
    let newTableHead = [];
    let newData = [];
    let colNames = undefined;

    if (filterData && filterData[0] && filterData[0].A === '列名') {
      try {
        colNames = JSON.parse(filterData[0].C)
      } catch (e) {

      }
      newData = [...filterData].slice(1, filterData.length).map((item, index) => {
        return { ...item, id: `${item.name}-${index}` }
      });
    } else {
      newData = filterData.map((item, index) => {
        return { ...item, id: `${item.name}-${index}` }
      });
    }
    if (colNames) {
      const include = tableHead.filter(col => col.dataIndex === 'dataIndex' || colNames[col.dataIndex]);
      const keys = include.map(col => col.dataIndex);
      newTableHead = include.map(col => {
        if (col.dataIndex === 'dataIndex') {
          return { ...col, title: col.dataIndex };
        } else if (colNames[col.dataIndex] === '列名') {
          return { ...col, title: col.dataIndex, style: { width: keys.includes('E') ? `${88 / (include.length - 1) - 4}%` : `${88 / (include.length - 1)}%` } }
        } else {
          return { ...col, title: col.dataIndex, name: colNames[col.dataIndex], style: { width: col.dataIndex === 'E' ? `${88 / (include.length) + 12}%` : keys.includes('E') ? `${88 / (include.length - 1) - 4}%` : `${88 / (include.length - 1)}%` } }
        }
      })
    } else {
      newTableHead = [...tableHead].map(col => ({ ...col, title: col.dataIndex }));
    }
      ...
      <div ref={this.ref} style={{ width: '100%', height: '100%', position: 'relative' }} className={styles.table_layout_case}>
        <table style={{ width: '100%' }}>
          <thead>
            <tr>
              {
                Array.isArray(newTableHead) && newTableHead.map((item, index) => {
                  let children = [];
                  let count = 0;
                  if (item.children) {
                    newData.map(key => {
                  ...
                  // <tr key={i} className={active_line} style={item.A === '总计' ? { position: 'absolute', bottom: 0 } : {}}>
                    <tr key={i} className={active_line}>
                    {
                      newTableHead.map((key, j) => {
                        ...
                        return (
                          !key.hidden ?
                            <td key={j} className={active_color} style={{ ...newTableHead[j]?.style }} onClick={key.tooltip ? this.tdClick.bind(this, i, j) : undefined}>
                              {/* {key.tooltip ?
                                <Tooltip
    ...
    return (
      <Fragment>
        {dataKey.tooltip ?
          ...
          typeof dataKey.render === 'function' ? dataKey.render(item[dataKey.title], item) : dataKey.title === 'ID' ? `${i + 1}` : item[dataKey.title]
        }
      </Fragment>
    );
  }
