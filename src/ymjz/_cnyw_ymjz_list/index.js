...
const dataHead = [
  { name: '序号', dataIndex: 'ID', style: { width: '6%' } },
  {
    name: '接种点', dataIndex: 'C', style: { width: '17%' }, tooltip: true, render: (text, record) => {
      console.log(text)
      const { B } = record || {};
  ...
  {
    name: '星期一', dataIndex: 'D', style: { width: '11%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '星期二', dataIndex: 'E', style: { width: '11%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '星期三', dataIndex: 'F', style: { width: '11%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '星期四', dataIndex: 'G', style: { width: '11%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '星期五', dataIndex: 'H', style: { width: '11%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '星期六', dataIndex: 'I', style: { width: '11%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
  {
    name: '星期日', dataIndex: 'J', style: { width: '11%' }, render: (text, record) => {
      let cnt = parseFloat(text);
      return (isNaN(cnt) ? 0 : cnt);
    }
  },
    ...
    return (
      <Fragment>
        {dataKey.tooltip ?
          ...
          typeof dataKey.render === 'function' && !item.flag ? dataKey.render(item[dataKey.title], item) : dataKey.title === 'ID' ? item[dataKey.title] || `${i + 1}` : item[dataKey.title]
        }
      </Fragment>
    );
  }
